...
var CHECK = 0;
var iter = 0; /*providers from json || doesn't work cross-server*/

var langName = []; /*LANGUAGES MAPPING*/
jQuery.ajax({
...
function initializeFinder() {
    if (!FINDER_INITIALIZED) {
        ...
        for (var i = 0; i < FACET_TOKENS.length; i++) {
            ...
            div.push('<a href="javascript:;" id="' + fn + '" onclick="return false;" class="filter_parent"><span>' + FACET_LABELS[fn] + '</span></a><div id="' + fn + '_rbo" class="filter_child" style="display: none; overflow: hidden;height:auto;"></div>');
        }
        div.push('</DIV>');
        ...
        onSuccess: function(transport) {
            ...
            if (initUpdate) {
                ...
                result.metadata.each(function(item, index) {
                    ...
                    {
                        if (item.format[0].indexOf('pdf') != -1) item.format = 'images/icons/pdf.png';
                        else if (item.format[0].indexOf('powerpoint') != -1) item.format = 'images/icons/ppt.png';
                        else if (item.format[0].indexOf('video') != -1) item.format = 'images/icons/video.png';
                        else if (item.format[0].indexOf('zip') != -1) item.format = 'images/icons/zip.png';
                        else if (item.format[0].indexOf('audio') != -1) item.format = 'images/icons/audio.png';
                        else if ((item.format[0].indexOf('text') != -1) || (item.format[0].indexOf('multipart') != -1)) item.format = 'images/icons/text.png';
                        else if ((item.format[0].indexOf('xml') != -1)) item.format = 'images/icons/xml.png';
                        else if (item.format[0].indexOf('image') != -1) item.format = 'images/icons/image.png';
                        //item.format=item.thumbnailUri;
                        //item.format=item.location;
                        else if ((item.format[0].indexOf('word') != -1) || (item.format[0].indexOf('wordprocessingml') != -1)) item.format = 'images/icons/word.png';
                        else if ((item.format[0].indexOf('application') != -1)) item.format = 'images/icons/application.png';
                    ...
                    if (item.descriptions != undefined) {
                        for (i = 0, tmpSize = item.descriptions.length; i < tmpSize; i++) {
                            if (item.descriptions[i].lang == SELECTED_LANGUAGE) item.thisDescription = item.descriptions[i].value;
                        }
                    }
                    ...
                    {
                        if (item.title[i].lang == SELECTED_LANGUAGE) item.thisTitle = item.title[i].value;
                    }
...
/**
*	Functions for Initializing JAML Templates
*/
function initializeJamlTemplates() {
	...
	*   FACETS
    */
    Jaml.register('rbcriteria', function(data) //FACETS OTHER THAN LANG 
    {
    ...
    Jaml.register('rbcriteria2', function(data) //LANG FACET
    {
        var label = data.val;
